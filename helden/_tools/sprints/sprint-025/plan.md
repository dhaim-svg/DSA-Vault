...
| T1 | **B-023** — Buch-Notation (`ZfP*`, `LkP*`, `RkP*`, `TaP*`) im Artikelparser neutralisieren (mistune-Inline-Regel), Tests + Korpus-Gegenprobe | ⬜ todo | parsers/wikiartikel.py, tests/test_wikiartikel.py |
...
- **B-023-Lösungsweg (User-Entscheidung): Parser-Vorbehandlung**, Wiki bleibt unberührt und buchtreu. Primärweg: eigene mistune-3-Inline-Regel für `(?:ZfP|LkP|RkP|TaP)\*` vor `emphasis` (greift nicht in Code-Spans/Fences; im Korpus 2 Fence-Zeilen + 1 Inline-Code-Vorkommen). Fallback: String-Vorbehandlung mit Fence-/Code-Ausschnitt — dann als Abweichung im Report begründen.
- **B-023-Abnahme am Korpus:** 123 → 0 render-relevante Zeilen; `<em>` im Static-Render 10 → 8, keines mit Buch-Notation; legitime Kursivschrift Zeichen für Zeichen unverändert. Die Pflegelisten `ROHSTERN_DATEIEN`/`ROHSTERN_ZEILEN` (B-020) auf geänderte Aussage prüfen.
