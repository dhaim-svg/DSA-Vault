...
- **B-023-Abnahme am Korpus (Ruling R4):** Stern-Bilanz je Ganzdatei (Quelle: Notation mit/ohne `\` vor `*` == literale `…*` im HTML): heute **82 Dateien falsch → 0**; `<em>` im Static-Render **16 → 14** (Ruling R5: der Pre-flight zählte 10 → 8 mit einer 80-Zeichen-Regex, die Differenz 2 stimmt), keines mit Buch-Notation; Lookahead `(?!\*)` in `_BOOK_STAR_RE`, damit `**LkP**` (Fettrand) nicht zerlegt wird; legitime Kursivschrift bleibt unverändert. Die Pflegelisten `ROHSTERN_DATEIEN`/`ROHSTERN_ZEILEN` (B-020) bleiben inhaltlich gültig (Sterne bleiben literal), nur die Aufrufstelle ändert sich.
...
- **D-052-Messbefund (T2, Rulings R6–R9):** Überlauf-Quelle ist das `.spell`-Grid der Zauberliste (Spalten 160/240/38/80/90/200 = 858 px inkl. Gaps gegen 733 px Innenbreite), **nicht** die Karte „Spontane Modifikationen“ (`.mod-table` bleibt innerhalb der Karte); im echten Druck (~703 px Seitenbreite) fehlen ~215 px. Fix: Druck-Grid mit `minmax(0,Xfr)`-Spalten (überlaufsicher durch Konstruktion), nicht Kompaktlayout. 20 Kontrast-Gruppen im Zauber-Tab < 4,5 : 1 (alle im Scope, R7); Slot-Schaltflächen im Druck ausgeblendet (R9); `.sf-name .meta` misst 4,28 (nicht 1,95), `.speicher-box` selbst 14,19 (die 1,04 kommen von `.slot-zauber`).
