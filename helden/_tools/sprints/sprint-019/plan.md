...
| T1 | **D-041a** Regelrecherche in den PDFs + Wiki-Artikel `grundregeln/zustaende.md` (Wundauswirkungen verlinkt, Zustände belegt oder als unbelegt markiert), `_grundregeln.md` + `_master-index.md` + `wiki-luecken.md` (L23) | ✅ done | wiki/dsa-4.1/grundregeln/zustaende.md (neu), wiki/dsa-4.1/grundregeln/_grundregeln.md, wiki/_master-index.md, wiki-luecken.md |
| T2 | **D-041b** Wundlogik regelkonform: Skopus-Modell statt Einzelzahl (`session.js`), `getWundMod(cfg)` nach Probenart (`dice.js`), Badge-Text nennt den Geltungsbereich, Overlay nur noch auf GE | ✅ done | static/session.js, static/dice.js, templates/partials/kampf.j2, tests/test_static_js.py |
| T3 | **D-041c** Zustände an T1 angleichen: belegte Zustände mit Quellwerten, unbelegte sichtbar als Hausregel, getrennt vom Wund-Malus | ✅ done | static/session.js, static/tabs.css, templates/partials/kampf.j2, tests/test_static_js.py |
| T4 | **D-046** Zauberliste: Print-Farben für `.nlink`/`.zfw-num`/`.zd`/`.kosten`/`.wirkung`, Grid-Überstand bei 1071–1130 px (`minmax`) | ✅ done | static/base.css, static/tabs.css, tests/test_rendering.py |
| T5 | **D-044** Mobile/Touch: Banner ≤ 400 px, Footer-Leiste (44 px + `position:static`), Steigern-Scroll `tabindex`, `.inv-add-input`-Klasse, Summary „▸ Artikel" 44 px | ✅ done | static/base.css, static/tabs.css, templates/partials/inventar.j2, tests/test_rendering.py |
| T6 | Verifikation (pytest inkl. `-W error`, Static-Render, **eine** Playwright-Runde 400/1071/1280 px + Print-Emulation, Domänen-Grenze) + Gesamt-Review, danach `/sprint-wrap` | ✅ done (Verifikation + Review; `/sprint-wrap` steht aus) | — |
...
- **T0–T6 fertig, 379/379 Tests** (Baseline 333, auch `-W error`). Commits `ab8a5d0`…`b98c547` (T0 `ab8a5d0`; T1 `3cb043e`+`d872643`; T2 `ebb702a`; T3 `89fd7ef`+`0aa7ef3`+`da46ea0`; T4 `37fee44`+`1300ffc`; T5 `ce26e50`+`9304983`; Final-Review `8603466`, `1970b15`; Print-Kopfzeile `1dec437`; Static-Render `e749739`, `b98c547`). **Nichts gepusht.**
- **Browser-Verifikation** (Playwright, Static-Render über `http.server` — nur lesend, nichts in den Vault geschrieben): **36 PASS / 0 FAIL / 1 nicht messbar** + Nachprüfung A8 **19 PASS / 0 FAIL**; Messwerte in `verification.md`. Kern: 2 Wunden → nur `GE 13→9` (Rest unverändert), AT/PA/FK/INI/GS `→` wie Regel, Panel AT/PA/GE −4, MU/KK/Talent/Zauber/Schaden 0, GE-Wert in Talent-/Zauberprobe 13→9; Druck-Kontrast 14,62:1; Grid-Überstand 0 px bei 1071/1100/1130/1280; alle 8 Tabs bei 400 px `scrollWidth == clientWidth == 385`; Footer statisch/44 px; Summary 44 px; Steigern-Scrollbereich per Tastatur scrollbar.
- **Gesamt-Review (Opus): „With fixes"** — 0 Critical; Important 2 (Wikilinks bare vs. relativ → **widerlegt**: Bare-Namen sind im Vault Konvention; GE-Abzug in Talent-/Zauberproben → behoben); Minor 7. Fix-Runde + scoped Re-Review: alle addressed.
- **Abweichungen von der Planskizze:** (1) neue `static/wundregeln.js` als gemeinsame Regelquelle (file://-Fallback, node-testbar) statt Tabelle in `session.js`; (2) `type:'schaden'` bekommt keinen Wundabzug mehr (Bestandsbug); (3) Basiswert-Overlay `base→eff` im Kampf-Tab (`data-wund-stat`); (4) **GE-Abzug pro Attribut in `parseProbe`** (Talent/Zauber) — weicht vom Wortlaut „Talent-/Zauberproben bleiben unmodifiziert" ab, hält aber deren Kern ein (kein pauschaler Malus, nur GE laut WdS S. 57) — **Entscheidungspunkt für den User**; (5) Steigern-Scrollbereich als Wrapper-`div` statt `role` am `<table>`, `steigern.js` zu T5; (6) Legende geteilt (Wund-Satz bleibt im Static-Render); (7) Print-Kopfzeile/Modifikations-Liste (Browser-Messung 1,95:1 / 4,28:1) mitgezogen; (8) Static-Render 434 594 → 446 203 B (+11,6 KB, Plan erwartete ±0).
- **Für `/sprint-wrap` vormerken:** D-041, D-044, D-046 → Done (Sprint 019); neues EPIC **D-047** (Touch-Höhen bei 400 px: Inventar-Eingaben/„+ Hinzufügen“ 30 px, Zustands-Chips 25 px); `wiki-luecken.md` L23 (Zustände nicht belegt) offen.
- **Bekannte Einschränkungen:** Zustands-Chips sind Hausregel (kein fester Probenmalus im Regelwerk, WdS/WdH/WdE geprüft; Wege der Zauberei/Zoo-Botanica nicht) und erscheinen weiter doppelt (Overlay + Panel-Vorbelegung, vorbestehend); optionale LE-/AU-Probenmali (WdS S. 57/83) bewusst nicht umgesetzt; Wundschwellen-Berechnung nicht im Dashboard; unter `file://` läuft `session.js` nicht (keine Overlays, aber Panel-Vorbelegung und GE-Attributwert über `DSAWundregeln`); `.zfw-num` im Druck unscoped; `.codex`-Padding 36 px (≤ 480 px) bei geöffnetem Würfelpanel ungemessen; je Steigern-Tabelle 1 zusätzlicher Tab-Stop + Scroll-Hinweis (≤ 600 px) auch wenn die Tabelle passt; Tests: Regex-Escapes `\b` werden bei Tool-Übergabe zu Steuerzeichen (ein vakuöser Test wurde gefunden und repariert — nach Tests immer `grep -nP '\x08|\x0c'`); Node-Verhaltenstests (`test_wundregeln.py`) laufen nur, wenn `node` vorhanden ist (sonst skip).
- **Unverändert offen (Out of Scope):** D-045, B-013, Vorschau für Rituale/SF, übrige Punkte aus Sprint-017/018-Handoff.
