...
| T4 | **B-022** — Quelle-Fallback im Artikelparser: fehlt `quelle` im Frontmatter, wird die `> **Quelle:** …`-Zeile des Artikels genutzt (wirkt für alle frontmatter-losen Kapitelartikel, z. B. alle `rituale/*`; **heute ist keine der 40 Vorschauen betroffen** — gemessen, s. Rulings) + Tests | ⬜ todo | parsers/wikiartikel.py, tests/test_wikiartikel.py |
...
- **Kein Frontmatter für `stabzauber.md`** (Ordner-Konvention: Kapitelartikel = Zitatblock). Stattdessen T4: `> **Quelle:**`-Zeile als Fallback für `_quelle(fm)` — behebt die leere Quelle für alle frontmatter-losen Kapitelartikel (nicht nur Stabzauber; die 15 SF-Vorschauen hatten sie schon, s. Korrektur unten).
- **Render-Erwartung je Task** (Golden-Baseline vor T1 ziehen): T1/T2 kein Diff · T3 Diff nur im Odem-Arcanum-Block (4 Zeilen fett statt roh) · T4 **kein** Diff (gemessen: alle 40 Vorschauböcke haben schon eine Quellenzeile; `sonderfertigkeiten/*-sonderfertigkeiten.md` tragen Frontmatter `quelle: WdZ`) · T6 **kein** Diff (`strip_wikilink()` führt Anzeigetext identisch; ein Diff wäre ein Fehler).
...
- **Korrektur nach Messung (Sprint 023 Start):** Die Plan-Aussage „behebt die leere Quelle auch für die 15 SF-Anker“ war falsch — 40/40 Vorschauböcke der Golden-Baseline tragen `artikel-quelle`. T4 bleibt (User-freigegeben; 127 von 809 Wiki-Artikeln haben kein Frontmatter, darunter alle `rituale/*` — Voraussetzung der Ritual-Vorschau D-051), ist aber ein reiner Parser-Vorbau ohne Render-Wirkung. Die Browser-Runde aus dem Freigabe-Plan (Punkt 7) hing an dieser Prämisse und entfällt, sofern T3 nicht mehr als den Odem-Arcanum-Block ändert.
