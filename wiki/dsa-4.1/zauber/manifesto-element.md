---
typ: zauber
name: MANIFESTO ELEMENT
alternativname: —
probe: KL/IN/CH (+Mod.)
komplexität: A
merkmale:
  - Elementar
kosten: 3 AsP
zauberdauer: 10 Aktionen
zielobjekt: Ort
reichweite: 1 Schritt
wirkungsdauer: "flüchtige: 1 Minute; bleibende: ZfP* SR"
repräsentationen:
  Ach: 7
  Dru: 5
  Geo: 5
  Mag: 5
  Hex: 3
  Srl: 3
  Elf: 2
quelle: LC
seite: 172
---

# MANIFESTO ELEMENT

> **Quelle:** LC S. 172–173

## Wirkung

Erzeugt kleine elementare Manifestationen (Funken, Wassertropfen, Windstoß, Erdklumpen). Flüchtige Effekte halten 1 Minute, bleibende Formen halten ZfP* SR.

## Varianten

- **Demanifesto** (+3): Löst kleine elementare Manifestationen auf.
- **Wirkungsdauer** (+5): Effekt hält bis Sonnenaufgang; 4 AsP Kosten.

## Reversalis / Antimagie

- Reversalis: Elementare Manifestation entsteht unkontrolliert.
- Antimagie: Standard.
